...
MAILFORGE_DIR="$(dirname "$SCRIPT_DIR")"
ROOT_DIR="$(dirname "$MAILFORGE_DIR")"

echo "What is your domain name? (e.g., kosh.uno)"
read DOMAIN

POSTGRES_PASSWORD="$(openssl rand -hex 32)"
POSTGRES_USER="postgres"

# Create MailForge .env file
cat > "$MAILFORGE_DIR/.env" << EOF
DOMAIN_NAME=${DOMAIN}
...
# Enable/disable email protocol servers (set to 'false' to disable)
...
echo ""
echo "2. Start the MailForge server:"
echo "   bun main.js"
